...
def build_vendor_escalation_info(vendor: str, vendor_result: dict | None = None,
    ...
    if is_cer:
        ...
        if reason and not reason_is_generic:
            if any(area in reason for area in ("九州", "中国", "四国", "対象エリア")):
                reason = "対象エリアのためCER候補。手配可否は担当確認が必要"
...
def case_basic_widget_key(field: str, revision: int | None = None, session_state=None) -> str:
    ...
    stem = CASE_BASIC_FIELD_TO_WIDGET_STEM.get(field)
    if stem is None:
        raise KeyError(f"Unknown case basic widget field: {field}")
...
def append_teams_send_log(result: dict, message: str, chat_name: str,
    ...
    destination_label = (
        destination_label
        or WARRANTY_REPORT_DESTINATION_LABELS.get(destination_key, "")
        or chat_name
    ).strip()
    entry = {
        "sent_at": timestamp,
...
def judge_script_route(form: dict) -> dict:
    ...
    store_text = " ".join(str(form.get(field) or "") for field in (
        ...
        "store_company", "operating_company",
        "call_line", "call_line_original", "line_name",
    ))
    company_text = " ".join(str(form.get(field) or "") for field in (
        "company_name", "store_company", "operating_company",
        "store_name", "manufacturer", "manufacturer_original",
...
def extract_fields_from_pasted_text(text: str) -> dict:
    ...
    field_specs = {
        ...
        "product_price": (["商品価格", "商品価格（円）", "商品価格 (円)", "商品金額", "購入金額", "税込価格", "価格"], r"([0-9,]+(?:[ \t　]*円)?)"),
        "genre": (["ジャンル"], r"([^\t\n]+)"),
        "category": (["分類"], r"([^\t\n]+)"),
...
def build_decision_tag_items(decision: dict, form: dict | None = None,
    ...
    wrs_action = decision.get("wrs_handover_action") or {}
    handover = decision.get("handover_requirement") or {}
    if wrs_action.get("needs_wrs_handover"):
        ...
        handover_tag = {
            "title": "引継要否",
            "primary": "必要",
            "secondary": f"依頼内容：{handover.get('handover_request_content') or '要確認'}",
            "tertiary": f"理由：{handover.get('reason') or handover.get('rule_name') or '要確認'}",
            "color": TAG_COLOR_WARNING,
        }
    else:
        handover_tag = {
            "title": "引継要否",
            "primary": "不要",
            "secondary": f"理由：{handover.get('reason') or '引き継ぎ対象ルールに一致なし'}",
        ...
        if not df_tpl.empty:
            ...
            for option_label, candidate in template_option_rows.items():
                if current_code and candidate.get("template_code") == current_code:
                    template_current_option = option_label
                    break
...
| CER候補 | 担当エスカ | 担当確認 |"""
    ...
    with teams_action_col:
        ...
        teams_enabled = bool(
            ...
            and destination.get("enabled")
            and destination.get("chat_id")
            and teams_send_mode in SUPPORTED_TEAMS_SEND_MODES
        )
        chat_name = destination.get("chat_name") or destination_label
        config_reasons = (
            warranty_teams_config_unavailable_reasons(teams_config, destination)
        ...
        def request_teams_send(allow_resend: bool = False):
            ...
            if is_warranty_destination:
                ...
                validation_errors = validate_teams_test_send_request(
                    {"teams_chat_message": message},
                    teams_config,
                    already_sent=(current_already_sent and not allow_resend),
                    destination=destination,
                )
            if validation_errors:
                st.warning(f"送信不可：{validation_errors[0]}")
                return
            if current_already_sent and not allow_resend:
        ...
        if in_progress:
            ...
            send_button_label = "Teamsワランティへ送信" if is_warranty_destination else "自分宛てにテスト送信"
            if st.button(send_button_label, disabled=not teams_enabled, type="primary", use_container_width=True):
